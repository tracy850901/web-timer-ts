...
/**
 * 會議計時小工具（React + 輕量 CSS）
 * ---------------------------------------------------------------------------
 * 修復：移除脫隊片段與錯置的程式碼（造成 Missing semicolon / return outside of function），
 *       重整檔案結構，確保 TypeScript 型別正確與建置通過。
 * 功能：分圈、開始/暫停/重置、目標分鐘比對、總時長、RWD、複製匯出（無括號）、
 *       本地保存（重整最多恢復 2 次）。
...
function renderTargetAndStatus(
  targetMinutes: number | undefined,
  durationMs: number
) {
  if (targetMinutes == null || Number.isNaN(targetMinutes))
    return <span className="text-gray-500">-</span>;
  const status = getPresetStatus(durationMs, targetMinutes);
  const statusCls = status === "超過" ? "text-red-600" : "text-blue-600";
  return (
    <span className="text-gray-700">
      目標：{targetMinutes} 分鐘 / <span className={statusCls}>{status}</span>
    </span>
  );
}
...
const buildTextSummary = (
  ...
  const lines = lapRows.map((lap, idx) => {
    ...
    return `${idx + 1}. ${fmtRange(lap.startTs)}~${fmtRange(
      lap.endTs
    )}，共 ${mins} 分鐘`;
  });
  if (rangeStart && rangeEnd) {
    lines.push("");
    lines.push(
      `總時長：${fmtRange(rangeStart)}~${fmtRange(rangeEnd)}（共 ${fmtHMS(
        rangeDuration || 0
      )}）`
    );
  }
...
async function safeCopyText(text: string) {
  try {
    ...
    warn("clipboard.writeText error:", err);
  }
  try {
    ...
    warn("execCommand copy error:", err);
    return false;
  }
}
...
function selectElementText(el: HTMLElement) {
  try {
    ...
    warn("selectElementText error:", err);
  }
}
...
function isTypingTarget(el: EventTarget | null) {
  ...
  const tag = anyEl.tagName ? anyEl.tagName.toLowerCase() : "";
  return (
    tag === "input" ||
    tag === "textarea" ||
    (anyEl as any).isContentEditable === true
  );
}

// 解析「區段目標時間（分鐘）」輸入（空字串不轉 0）
function computePresetFromInput(
  raw: string,
  valueAsNumber?: number
): number | undefined {
  if (raw.trim() === "") return undefined; // 讓受控 input 顯示 "" 並維持 caret 閃動
  const n = valueAsNumber;
...
function useFitFont<C extends HTMLElement, T extends HTMLElement>(
...
export default function App() {
   ...
   * - sessionStartTsRef/lapAnchorTsRef/activeStartTsRef：總時長起點、當前圈起點（顯示列）
  ...
  const sessionStartTsRef = useRef<number | null>(null); // 第一次開始（總時長 start）
  ...
  const [currentTargetMinutes, setCurrentTargetMinutes] = useState<
    number | undefined
  >(undefined);
  ...
  useEffect(() => {
    try {
      ...
      let left = Number.isFinite(parseInt(leftRaw || "", 10))
        ? parseInt(leftRaw!, 10)
        : MAX_RELOADS;
      setReloadsLeft(left);
      ...
      if (left > 0) {
        ...
        if (raw) {
          ...
          if (parsed) {
            ...
            if (typeof parsed.baseElapsed === "number")
              setBaseElapsed(parsed.baseElapsed);
            if (typeof parsed.sessionStartTs === "number")
              sessionStartTsRef.current = parsed.sessionStartTs;
            if (typeof parsed.lapAnchorTs === "number")
              lapAnchorTsRef.current = parsed.lapAnchorTs;
            if (typeof parsed.activeStartTs === "number")
              activeStartTsRef.current = parsed.activeStartTs;
            if (typeof parsed.lastPauseTs === "number")
              lastPauseTsRef.current = parsed.lastPauseTs;
            if (
              parsed.activeTargetMinutes != null &&
              !Number.isNaN(parsed.activeTargetMinutes)
            ) {
              activeTargetMinutesRef.current = parsed.activeTargetMinutes;
            }
            if (
              parsed.currentTargetMinutes != null &&
              !Number.isNaN(parsed.currentTargetMinutes)
            ) {
          ...
          if (rawV5) {
            try {
              ...
              if (v5) {
                ...
                if (typeof v5.baseElapsed === "number")
                  setBaseElapsed(v5.baseElapsed);
                sessionStartTsRef.current =
                  typeof v5.sessionStartTs === "number"
                    ? v5.sessionStartTs
                    : null;
                lapAnchorTsRef.current =
                  typeof v5.lapAnchorTs === "number"
                    ? v5.lapAnchorTs
                    : sessionStartTsRef.current;
                activeStartTsRef.current =
                  lapAnchorTsRef.current ?? sessionStartTsRef.current;
                lastPauseTsRef.current =
                  typeof v5.lastPauseTs === "number" ? v5.lastPauseTs : null;
                if (
                  v5.currentTargetMinutes != null &&
                  !Number.isNaN(v5.currentTargetMinutes)
                )
                  setCurrentTargetMinutes(v5.currentTargetMinutes);
                log("restore:migrated from v5");
              }
            } catch (err) {
              warn("migrate v5 error", err);
            }
          } else {
            ...
            if (legacyRaw) {
              try {
                ...
                if (legacy && Array.isArray(legacy.segments)) {
                  ...
                  setLaps(migrated);
                  setBaseElapsed(
                    typeof legacy.baseElapsed === "number"
                      ? legacy.baseElapsed
                      : 0
                  );
                  sessionStartTsRef.current = migrated.length
                    ? Math.min(...migrated.map((l) => l.startTs))
                    : null;
                  const lastEnd = migrated.length
                    ? Math.max(...migrated.map((l) => l.endTs))
                    : null;
  ...
  const elapsed = useMemo(() => {
    ...
    if (val !== prevElapsedRef.current)
      debug("elapsed update (monotonic)", {
        from: prevElapsedRef.current,
        to: val,
      });
    prevElapsedRef.current = val;
    return val;
  }, [rawElapsed]);
  ...
  const lapsAccumulatedMs = useMemo(
    () => laps.reduce((acc, l) => acc + (l.durationMs || 0), 0),
    [laps]
  );
  ...
  const handleStart = () => {
    ...
    if (sessionStartTsRef.current == null) {
      ...
      lapAnchorTsRef.current = now;
      activeStartTsRef.current = now;
      ...
      if (activeStartTsRef.current == null) {
        activeStartTsRef.current =
          lapAnchorTsRef.current ?? sessionStartTsRef.current ?? now;
      }
  ...
  const handlePause = () => {
    ...
    setBaseElapsed((v) => {
      ...
      const next = Math.max(displayedElapsed, computed);
      log("pause:", { now, computed, displayedElapsed, next });
      return next;
    });
    startedAtRef.current = null;
    lastPauseTsRef.current = now; // 凍結 activeRow 的 end 顯示
  };

  const handleReset = () => {
    if (isRunning) {
      warn("reset:ignored (RUNNING)");
      return;
    }
    setIsRunning(false);
    setBaseElapsed(0);
    ...
    lapAnchorTsRef.current = null;
    activeStartTsRef.current = null;
    lastPauseTsRef.current = null;
    activeTargetMinutesRef.current = undefined;
    setCurrentTargetMinutes(undefined);
    localStorage.removeItem(STORAGE_KEY_STATE);
    resetReloadBudget();
    prevElapsedRef.current = 0;
    prevRangeDurRef.current = 0;
    log("reset:done");
  };

  const handleLap = () => {
    if (!isRunning) {
      warn("lap:ignored (not RUNNING)");
      return;
    }
    if (sessionStartTsRef.current == null) {
      warn("lap:ignored (session not started)");
      return;
    }
    const now = Date.now();
    ...
    const startTs =
      ...
      now;
    const endTs = now;
    const targetMinutes = activeTargetMinutesRef.current; // 用開圈時鎖定的目標
    const newLap = { startTs, endTs, durationMs: currentLapMs, targetMinutes };
    setLaps((arr) => [...arr, newLap]);
    lapAnchorTsRef.current = endTs;
    activeStartTsRef.current = endTs; // 開啟下一圈
    activeTargetMinutesRef.current = currentTargetMinutes; // 🔒 鎖定下一圈目標
    log("lap:add", newLap);
  };

  // ---- 表格資料 ----
  const completedRows = laps;
  const hasActive = activeStartTsRef.current != null;
  const completedRowsDesc = useMemo(
  ...
  const activeRow = useMemo(() => {
    ...
    const start = activeStartTsRef.current!;
    const end = isRunning ? Date.now() : lastPauseTsRef.current ?? Date.now();
    const durationMs = Math.max(0, elapsed - lapsAccumulatedMs);
    const targetMinutes = activeTargetMinutesRef.current;
  ...
  const displayRangeDuration = useMemo(() => {
    const val = Math.max(prevRangeDurRef.current, rangeDuration); // 總時長顯示單調
    if (val !== prevRangeDurRef.current)
      debug("rangeDuration update (monotonic)", {
        from: prevRangeDurRef.current,
        to: val,
      });
    prevRangeDurRef.current = val;
    return val;
  }, [rangeDuration]);
  ...
  const handleCopy = async () => {
    ...
    if (ok) {
      ...
      if (summaryRef.current)
        selectElementText(summaryRef.current as unknown as HTMLElement);
      setCopyStatus("fail");
  ...
  const handlePresetChange = (
    ...
    const target = computePresetFromInput(
      raw,
      ev?.currentTarget?.valueAsNumber
    );
    setCurrentTargetMinutes(target);
    log("preset:set currentTargetMinutes=", target);
  };

  // 快捷鍵：Space 開始/暫停/恢復；L 分圈；R 重置
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const assertEq = (a: any, b: any, msg: string) => {
      if (a !== b) console.error("[TEST FAIL]", msg, "expected:", b, "got:", a);
    ...
    const t = msToHMS(61500);
    assertEq(`${t.h}:${t.m}:${t.s}`, "00:01:01", "msToHMS(61500)");
    ...
    // 2b. 預設輸入解析（解法A）
    assertEq(
      computePresetFromInput("", Number.NaN),
      undefined,
      "empty -> undefined"
    );
    assertEq(
    ...
    // 3b. Label 應為「總時長」而非「總範圍」
    assertEq(summary.includes("總時長："), true, "summary uses 總時長 label");
    assertEq(summary.includes("總範圍："), false, "summary no 總範圍 label");

    // 4. 暫停不計入圈時長（理論模擬）
    (function testLapExcludesPause() {
      const resume = 8_000;
      const now = 13_000; // 有效 = [0..3] + [8..13] = 8s
      const baseElapsedSim = 3_000;
      const runningDelta = now - resume;
      const elapsedSim = baseElapsedSim + runningDelta; // 8_000
      const lapsAccumBefore = 0;
      const currentLapMs = Math.max(0, elapsedSim - lapsAccumBefore);
    ...
    (function testPresetLockedPerLap() {
      const lockedAtStart = 1;
      const changedLater = 2;
      const lapRecordedTarget = lockedAtStart;
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6">
        ...
        <section className="rounded-2xl p-4 sm:p-8 shadow-sm border bg-gray-50 border-gray-200">
          <div className="text-center">
            ...
            {rangeStart != null && rangeEnd != null && (
              <div className="mt-1 text-sm text-gray-600">
                總時長：{fmtRange(rangeStart)}~{fmtRange(rangeEnd)}（共{" "}
                {fmtHMS(displayRangeDuration)}）
              </div>
            )}
          </div>
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-3 justify-center">
            {isRunning ? (
              <button
                onClick={handlePause}
                className="w-full sm:w-auto min-h-12 px-4 py-2 rounded-2xl border"
              >
                暫停 (Space)
              </button>
            ) : (
            ...
            <button
              onClick={handleLap}
              disabled={!isRunning}
              title={!isRunning ? "暫停時不可分圈" : undefined}
              className="w-full sm:w-auto min-h-12 px-4 py-2 rounded-2xl border disabled:opacity-50 disabled:cursor-not-allowed"
            >
              分圈 (L)
            </button>
            <button
              onClick={handleReset}
              disabled={isRunning}
              title={isRunning ? "RUNNING 狀態無法重置，請先暫停" : undefined}
              className="w-full sm:w-auto min-h-12 px-4 py-2 rounded-2xl border disabled:opacity-50 disabled:cursor-not-allowed"
            >
              重置 (R)
            </button>
              ...
              <table className="w-full text-xs sm:text-sm min-w-[560px]">
                ...
                <tbody>
                  {hasActive && activeRow && (
                    <tr className="border-t bg-gray-50/50">
                      ...
                      <td className="px-4 py-2 whitespace-nowrap">
                        {fmtRange(activeRow.startTs)} ~{" "}
                        {fmtRange(activeRow.endTs)}
                        <span className="ml-2 text-xs text-gray-500">
                          （{isRunning ? "進行中" : "暫停"}）
                        </span>
                      </td>
                      <td className="px-4 py-2 font-mono">
                        {fmtHMS(activeRow.durationMs)}
                      </td>
                      <td className="px-4 py-2">
                        {renderTargetAndStatus(
                          activeRow.targetMinutes,
                  ...
                  {completedRowsDesc.map((lap, idx) => {
                    ...
                    const rowNumber = completedCount - idx; // 底部=1
                    return (
                      <tr key={idx} className="border-t">
                        <td className="px-4 py-2">{rowNumber}</td>
                        <td className="px-4 py-2 whitespace-nowrap">
                          {fmtRange(lap.startTs)} ~ {fmtRange(lap.endTs)}
                        </td>
                        <td className="px-4 py-2 font-mono">
                          {fmtHMS(lap.durationMs)}
                        </td>
                        <td className="px-4 py-2">
                          {renderTargetAndStatus(
                            lap.targetMinutes,
        ...
        <section className="mt-6">
          ...
          <div className="mb-2 flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
            <button
              onClick={handleCopy}
              className="w-full sm:w-auto px-4 py-2 rounded-2xl border"
            >
              複製文字總結
            </button>
            {copyStatus === "ok" && (
            ...
            {copyStatus === "fail" && (
              <span className="text-amber-600 text-sm">
                無法自動複製，已為你選取文字，請按 ⌘/Ctrl + C
              </span>
            )}
          </div>
          <pre
            ref={summaryRef as any}
            className="bg-gray-100 p-3 rounded text-xs sm:text-sm whitespace-pre-wrap overflow-x-auto"
          >
            {summaryText}
          </pre>
        </section>
        ...
        <p className="text-xs text-gray-500">
          *本地保存：同一輪最多可恢復 2 次重新整理；超過後不再保存當次紀錄。
        </p>
      </div>
    </div>
  );
...
function TimerReadout({
  ...
  const maxPx = emphasized ? 72 : 40;
  const minPx = emphasized ? 24 : 16;
  ...
  return (
    <div ref={containerRef} className="w-full overflow-hidden px-1">
      <div
        ref={textRef}
        className="font-mono tracking-normal sm:tracking-wider select-none whitespace-nowrap mx-auto text-center"
